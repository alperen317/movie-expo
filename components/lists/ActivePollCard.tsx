...
export function ActivePollCard({ deadline, candidates, onVote }: ActivePollCardProps) {
  ...
  // A poll that's still running opens expanded so votes are one tap away;
  // a closed one collapses so the header's winner summary (below) is the
  // whole story and the candidate list doesn't eat screen space by default.
  const [expanded, setExpanded] = useState(() => !isClosed);

  const winner =
    isClosed && maxVotes > 0
      ? candidates.reduce((best, c) =>
          c.candidate.voteCount > best.candidate.voteCount ? c : best,
        )
      : null;
  const headerSummary = isClosed
    ? winner
      ? `${winner.item.title} · ${t('components.activePoll.voteCount', { count: winner.candidate.voteCount })}`
      : t('components.activePoll.closed')
  ...
  return (
    <View className="mx-margin-mobile mb-stack-md gap-stack-sm rounded-2xl border border-glass-border bg-surface-container-low p-4">
      <Pressable
        ...
        accessibilityLabel={t(expanded ? 'a11y.collapsePoll' : 'a11y.expandPoll')}
      ...
      >
        ...
        <View className="ml-2 flex-shrink flex-row items-center gap-1">
          <Text
            className="flex-shrink font-sans text-caption text-text-secondary"
            numberOfLines={1}
          >
            {headerSummary}
          </Text>
