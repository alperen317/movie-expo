import { getTrailerCandidates, nextTrailerAction, youtubeWatchUrl } from './trailer';
...
describe('nextTrailerAction', () => {
  it('advances to the next candidate while one is left', () => {
    expect(nextTrailerAction(['a', 'b', 'c'], 0)).toEqual({ type: 'retry', index: 1 });
    expect(nextTrailerAction(['a', 'b', 'c'], 1)).toEqual({ type: 'retry', index: 2 });
  });

  // The last candidate failing means embedding is not the fixable part any
  // more, so hand off to YouTube itself.
  it('falls back to the first candidate externally once every clip has failed', () => {
    expect(nextTrailerAction(['a', 'b', 'c'], 2)).toEqual({ type: 'external', key: 'a' });
